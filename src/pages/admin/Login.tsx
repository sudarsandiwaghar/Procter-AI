...
import { Shield, Eye, EyeOff, Mail, Lock, Check, ArrowLeft } from "lucide-react";
...
export default function AdminLogin({ registeredUsers, onLoginSuccess }: LoginProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (matchedUser) {
      if (matchedUser.role === "admin") {
        setSuccess("Authentication success! Initiating secure Systems Admin dashboard...");
        setTimeout(() => {
      ...
      setError("Invalid Admin credentials. (Hint: Use sudarsan@ssit.edu / Root)");
    }
  };
      ...
      <div className="w-full max-w-md bg-neutral-900/40 border border-white/10 rounded-2xl p-8 shadow-2xl relative overflow-hidden backdrop-blur-md">
        ...
        <div className="text-center space-y-3 mb-8">
          ...
          <div>
            ...
            <p className="text-xs text-white/40 font-mono uppercase tracking-widest mt-1">Systems Operations</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 text-left font-space text-xs">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <button 
                type="button"
                onClick={() => alert("Administrative access cannot be recovered client-side. Please coordinate via Root Terminal.")}
        ...
        <div className="mt-8 border-t border-white/5 pt-6 text-center text-[10px] text-white/40">
          Admin registration is strictly forbidden. Systems Operations credentials are locked and managed via the server secure blueprint configuration.
        </div>
      </div>
    </div>
  );
