...
import { Shield, Eye, EyeOff, Mail, Lock, Award, Check, ArrowLeft } from "lucide-react";
...
export default function FacultyLogin({ registeredUsers, onLoginSuccess }: LoginProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!u.includes("@")) {
      setError("Please enter a valid institutional Mail ID (e.g. professor@ssit.edu).");
      return;
    }
    ...
    if (matchedUser) {
      if (matchedUser.role === "faculty") {
        setSuccess("Authentication success! Initiating Faculty Instructor dashboard...");
        setTimeout(() => {
      ...
      setError("Invalid Faculty credentials. (Hint: Use faculty@ssit.edu / Root)");
    }
  };
    ...
    <div className="min-h-screen bg-[#020202] text-white flex flex-col justify-center items-center px-4 py-12 relative selection:bg-emerald-500 selection:text-black">
      ...
      <div className="w-full max-w-md bg-neutral-900/40 border border-white/10 rounded-2xl p-8 shadow-2xl relative overflow-hidden backdrop-blur-md">
        ...
        <div className="text-center space-y-3 mb-8">
          ...
          <div>
            <h2 className="font-serif text-3xl font-bold text-white tracking-tight">Faculty Portal</h2>
            <p className="text-xs text-white/40 font-mono uppercase tracking-widest mt-1">Instructor Workspace</p>
          </div>
        </div>
        ...
        <div className="mt-8 border-t border-white/5 pt-6 text-center text-[10px] text-white/40">
          Faculty registration is restricted. Instructor credentials must be provisioned by the systems administrator.
        </div>
      </div>
    </div>
  );
